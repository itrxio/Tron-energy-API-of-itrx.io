...
所有POST请求，必须签名后发送请求，GET请求无需签名，仅需要在Header中带入API-KEY即可

## POST签名过程

首先，确保您已经获取到了[API](/100-before.md)，如果还未获取API，请先注册获取，下面我们以下单接口为例，完整演示签名过程

- **准备**


确定使用接口URL，这里演示使用下单接口
```
...
- **步骤 2**: 添加时间戳
我们将当前的时间戳（精确到秒, 10位整数），添加到第一步得到参数之前，得到如下字符串
```
...
- **步骤 3**: 使用API_SECRET加密
对步骤2的字符串用API_SECRET做sha256加密，得到
```
'07c125cabfe006614a02131f927cca9e7a654f7644a817b155fad591e1c50f7b'
```
::: tip

- 测试时，您可以直接用文档中步骤2的结果加密，看是否和步骤3的结果一致
  :::

- **步骤 4**:  发送请求，将`API-KEY（和API_SECRET成对）`， `TIMESTAMP（同步骤2中的时间戳一致）`，`SIGNATURE(步骤3的结果)`添加到请求头
```json
  headers = {
...
$result = file_get_contents("https://itrx.io/api/v1/frontend/order", false, $context);
var_dump($result);
?>
